...
class ModelCatalogProduct extends Model {
	...
	public function getProducts($data = array()) {
        ...
        if (!empty($data['filter_attributes'])) {
            $sql .=  " AND pa.attribute_id IN(" . $data['filter_attributes'] . ")";
        }
		...
		if (!empty($data['filter_name']) || !empty($data['filter_tag']) || !empty($data['search_attributes'])) {
			$sql .= " AND (";
            ...
            if (!empty($data['filter_name']) && !empty($data['search_attributes'])) {
                $sql .= " OR ";
            }

            if (!empty($data['search_attributes'])) {

                $implode = array();

                $words = explode(' ', trim(preg_replace('/\s+/', ' ', $data['search_attributes'])));
	...
	public function getProductAttributes($product_id) {
		...
		foreach ($product_attribute_group_query->rows as $product_attribute_group) {
			...
			$product_attribute_query = $this->db->query("SELECT a.attribute_id, ad.name, pa.text, ad.image FROM " . DB_PREFIX . "product_attribute pa LEFT JOIN " . DB_PREFIX . "attribute a ON (pa.attribute_id = a.attribute_id) LEFT JOIN " . DB_PREFIX . "attribute_description ad ON (a.attribute_id = ad.attribute_id) WHERE pa.product_id = '" . (int)$product_id . "' AND a.attribute_group_id = '" . (int)$product_attribute_group['attribute_group_id'] . "' AND ad.language_id = '" . (int)$this->config->get('config_language_id') . "' AND pa.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY a.sort_order, ad.name");
